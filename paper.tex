...
\affiliation{Independent researcher \\ \email{taras@dappros.com}}
...
Analysis benefited substantially from work performed by the Anthropic Claude language model (Opus 4.7) acting as a research assistant: code authoring, milestone-by-milestone analysis, detection of methodological errors (kernel pathology; BIC formula bug; sampler-convergence bias), draft preparation, and iterative critique. Per AAS journal policy, Claude is not listed as an author. The science responsibility for this work rests entirely with the human authors. The complete development log (23 milestone reports, $\sim$5\,500 lines of analysis code, source repository) is publicly available at \texttt{https://github.com/phwizard/beholder-tau-ceti} (DOI to be assigned via Zenodo).

This research has made use of the DACE platform developed in the frame of PlanetS (\texttt{https://dace.unige.ch}), and of \texttt{numpy}, \texttt{scipy}, \texttt{pandas}, \texttt{matplotlib}, \texttt{astropy}, \texttt{celerite2}, \texttt{emcee}, \texttt{zeus-mcmc}, \texttt{dynesty}, and \texttt{RadVel}.
